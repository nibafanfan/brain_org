...
def main() -> None:
    ...
    cls_col = _first_present(["CellClass_cal", "CellClass_pred", "cell_class_pred", "mapped_class"], adata.obs.columns)
    reg_col = _first_present(["Region_pred", "region_pred", "mapped_region"], adata.obs.columns)
    conf_col = _first_present(
        ["CellClass_cal_conf", "CellClass_conf", "cell_class_conf", "confidence", "calibrated_confidence"],
        adata.obs.columns,
    )
    ...
    df[cls_col] = df[cls_col].astype(str)                     # drop Categorical (breaks < comparisons)
    df[reg_col] = df[reg_col].astype(str)
    df["age_days"] = pd.to_numeric(df["age_days"], errors="coerce")
    if "dataset" not in df.columns and "dataset_slug" in df.columns:
        df["dataset"] = df["dataset_slug"].astype(str)        # per-dataset grouping (ours)
    print(f"[figure2] cls={cls_col} reg={reg_col} conf={conf_col} stage={stage_col} umap={umap_key}", flush=True)
    ...
    cls_prop = (df[cls_col].value_counts(normalize=True)
                .rename_axis("cell_class").reset_index(name="prop"))
